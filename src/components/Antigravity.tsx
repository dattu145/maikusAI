...
const AntigravityInner = ({
  ...
  // Track mouse on the WINDOW so particles respond even when hovering over
  // overlaid cards/text (not just the canvas element itself)
  const windowNDC = useRef({ x: 0, y: 0 });
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      windowNDC.current.x = (e.clientX / window.innerWidth) * 2 - 1;
      windowNDC.current.y = -((e.clientY / window.innerHeight) * 2 - 1);
    };
  ...
  useFrame(state => {
    ...
    // Use window-level NDC so particles react even behind overlaid HTML elements
    const m = windowNDC.current;
