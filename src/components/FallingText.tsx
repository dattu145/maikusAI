...
const FallingText = ({
  ...
  // External trigger: fires when forceStart becomes true
  useEffect(() => {
    if (forceStart && !effectStarted) setEffectStarted(true);
  }, [forceStart]);
  ...
  useEffect(() => {
    ...
    World.add(engine.world, [floor, leftW, rightW, ceiling, mc, ...wordBodies.map(w => w.body)]);
  ...
  return (
    ...
    >
      <div ref={textRef} className="falling-text-target" style={{ fontSize, lineHeight: 1.5 }} />
