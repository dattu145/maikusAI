...
class Geometry {
  ...
  subdivide(divisions = 1) {
    const midPointCache = {}; let f = this.faces;
...
function createProgram(gl, shaderSources, tfv, attribLocations) {
  const program = gl.createProgram();
  [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((type, ndx) => {
    const shader = createShader(gl, type, shaderSources[ndx]);
    if (shader) gl.attachShader(program, shader);
  });
  if (tfv) gl.transformFeedbackVaryings(program, tfv, gl.SEPARATE_ATTRIBS);
...
class ArcballControl {
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    let af = ts, snapRotation = quat.create();
    if (this.isPointerDown) {
      const INT = 0.18 * ts, AA = 3.5 / ts;
      ...
      if (this.snapTargetDirection) {
        const SI = 0.07, a = this.snapTargetDirection, b = this.snapDirection;
        const sqrDist = vec3.squaredDistance(a, b), df = Math.max(0.05, 1 - sqrDist * 8);
        af *= SI * df; this.quatFromVectors(a, b, snapRotation, af);
      }
    }
    const cq = quat.multiply(quat.create(), snapRotation, this.pointerRotation);
  ...
  quatFromVectors(a, b, out, angleFactor = 1) {
    ...
    return { q: out, axis, angle: Math.acos(d) * angleFactor };
  }
  #project(pos) {
...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Prefill with a dark placeholder so sphere shows immediately
  ...
  #animate(deltaTime) {
    ...
    this.instancePositions.map(p => vec3.transformQuat(vec3.create(), p, this.control.orientation)).forEach((p, ndx) => {
      const s = (Math.abs(p[2]) / this.SPHERE_RADIUS) * SI + (1 - SI), fs = s * scale;
      const m = mat4.create();
  ...
  #render() {
    ...
    // Brand bg: #060A18 = rgb(6,10,24)
    // Switch bg color based on active theme class
    if (document.documentElement.classList.contains('light')) {
      gl.clearColor(0.973, 0.980, 0.988, 1.0); // #f8fafc light bg
    } else {
      gl.clearColor(0.024, 0.039, 0.094, 1.0); // #060A18 dark bg
    }
  ...
  #updateProjectionMatrix(gl) {
    ...
    // Use full sphere radius so all discs are visible (not a zoomed-in blob)
    const h = this.SPHERE_RADIUS * 0.92, d = this.camera.position[2];
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      const ni = this.#findNearestVertexIndex(); this.onActiveItemChange(ni % Math.max(1, this.items.length));
      this.control.snapTargetDirection = vec3.normalize(vec3.create(), this.#getVertexWorldPosition(ni));
    } else { ctz += this.control.rotationVelocity * 60 + 1.5; damping = 12 / ts; }
    this.camera.position[2] += (ctz - this.camera.position[2]) / damping; this.#updateCameraMatrix();
  }
  #findNearestVertexIndex() {
    const n = this.control.snapDirection, inv = quat.conjugate(quat.create(), this.control.orientation);
    const nt = vec3.transformQuat(vec3.create(), n, inv); let maxD = -1, ni;
    for (let i = 0; i < this.instancePositions.length; ++i) { const d = vec3.dot(nt, this.instancePositions[i]); if (d > maxD) { maxD = d; ni = i; } }
    return ni;
  }
  #getVertexWorldPosition(index) { return vec3.transformQuat(vec3.create(), this.instancePositions[index], this.control.orientation); }

  // Rotate the globe so the next testimonial faces the camera
  goToNextItem(currentItemIndex, totalItems) {
    if (!totalItems) return;
    const nextIndex = (currentItemIndex + 1) % totalItems;
    const snapDir = vec3.fromValues(0, 0, -1); // "front" direction in this sphere's convention

    // Find vertex mapping to nextIndex that is currently most visible (nearest to front)
    let bestScore = -Infinity;
    let bestWorldPos = null;
    for (let i = 0; i < this.instancePositions.length; i++) {
      if (i % totalItems !== nextIndex) continue;
      const wp = vec3.normalize(vec3.create(), this.#getVertexWorldPosition(i));
      const score = vec3.dot(wp, snapDir);
      if (score > bestScore) { bestScore = score; bestWorldPos = wp; }
    }
    if (!bestWorldPos) return;

    // Build rotation that brings bestWorldPos to [0,0,-1]
    const axis = vec3.normalize(vec3.create(), vec3.cross(vec3.create(), bestWorldPos, snapDir));
    if (vec3.length(axis) < 0.001) return;
    const angle = Math.acos(Math.max(-1, Math.min(1, vec3.dot(bestWorldPos, snapDir))));
    const rotQ = quat.setAxisAngle(quat.create(), axis, angle);

    const newOri = quat.normalize(quat.create(), quat.multiply(quat.create(), rotQ, this.control.orientation));
    this.control.orientation = newOri;
    // Give the control some velocity so the animation feels smooth
    quat.slerp(this.control.pointerRotation, this.control.pointerRotation, rotQ, 0.35);
  }
}

const defaultItems = [{ image: 'https://picsum.photos/600/600', link: '#', title: 'Item', description: '' }];
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    let sketch;
    try {
      sketch = new InfiniteGridMenu(canvas, items.length ? items : defaultItems, idx => setActiveItem(items[idx % items.length]), setIsMoving, sk => sk.run(), scale);
      sketchRef.current = sketch;
    } catch (e) { console.error('InfiniteMenu init failed:', e); }
  ...
  const handleButtonClick = () => {
    if (!sketchRef.current || !items.length) return;
    const currentIndex = activeItem ? items.indexOf(activeItem) : 0;
    sketchRef.current.goToNextItem(currentIndex, items.length);
  };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <div className={`infinite-face-title ${isMoving ? 'inactive' : 'active'}`}>
            <span className="inf-name">{activeItem.name}</span>
            <span className="inf-role">{activeItem.role}</span>
            <div className="infinite-stars">
              {[1,2,3,4,5].map(i => <span key={i}>★</span>)}
            </div>
          </div>

          <p className={`infinite-face-description ${isMoving ? 'inactive' : 'active'}`}>
            {activeItem.description}
          </p>

          {activeItem.link && activeItem.link !== '#' && (
            <div onClick={handleButtonClick} className={`infinite-action-button ${isMoving ? 'inactive' : 'active'}`}>
              <p className="infinite-action-button-icon">&#x2197;</p>
            </div>
          )}
        </>
      )}
